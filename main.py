def InputFile(namafile):
    ...
    connected = []
    print()
    for i in range (int(arrFile[0])+1, len(arrFile)):
        for j in range (len(arrFile[i])):
            if (arrFile[i][j] != '0' or arrFile[i][j] != ' '):
                connected.append([ArrSimpul[i+1-int(arrFile[0])][0],arrFile[i][j]])
...
def main():
    ...
    path = astar_search(graph, heuristics, node1, node2)
    print("Path :", path)
    print()
